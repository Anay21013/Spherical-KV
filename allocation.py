...
def _effective_bits(tier) -> int:
    """
    Effective bits per token including page alignment overhead (App A.2).
    R_eff(t) = token_bits(t) + PAGE_ALIGNMENT_BITS_PER_TOKEN
    """
    tb = tier.token_bits()
    if tb == 0:
        return 0
...
def online_refresh(retained_tokens: list, tiers: list,
                   prefill_len: int = 0) -> list:
    """
    Decode-time controller refresh (C.3).
    Now includes Lagrangian prices for per-layer/per-head caps and
    page alignment overhead in budget accounting.
    """
    if not retained_tokens:
        return retained_tokens
...
def _compute_lagrange_prices(ctrl_tokens, cur_tier, tier_by_id, B_eff):
    """
    Compute Lagrangian prices for per-layer and per-head caps (C.3).
    Uses projected subgradient with fixed step size and clipping.
    Returns dict with 'layer' and 'head' price dicts.
    """
