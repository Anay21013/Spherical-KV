...
def _call_fused(
    ...
    positions:     torch.Tensor,    # [P * PAGE_SIZE] int32, -1 for pad
    cos_table:     torch.Tensor,    # [max_pos, dh] float32
    sin_table:     torch.Tensor,    # [max_pos, dh] float32
...
class SphericalKVPipeline:
    ...
    def prefill(
        ...
        kv_pairs, attn_list, ho_list, logits, pre_rope_K_list = capture_prefill_pass(
        ...
        for li, (K, V) in enumerate(kv_pairs):
            ...
            K_pre_layer = pre_rope_K_list[li][0].view(
    ...
    def _append_decode_kv(
        ...
        r_groups_fine = k_grouped.norm(dim=-1).cpu()      # [G_fine] float32
        ...
        pos_t = torch.tensor([pos_val], dtype=torch.long)
